...
document.addEventListener("DOMContentLoaded", function () {
	...
	function initializeWebSocket() {
		async function fetchAndUpdateCows() {
			try {
				...
				// Cows placed outside the fence boundary for demo
				const outOfFenceIds = [2, 7, 8];
				const outOfFencePos = {
					2: { lat: 49.9286, lon: 16.6058 },  // north of fence
					7: { lat: 49.9254, lon: 16.6015 },  // west of fence
					8: { lat: 49.9244, lon: 16.6104 }   // east of fence
				};
				const mockData = {
					data: cowData.map(cow => {
						...
						const oof = outOfFenceIds.includes(cow.id);
						return {
							id: cow.id,
							name: collarInfo?.animal?.name || `Cow ${cow.id}`,
							gps_lat: oof ? outOfFencePos[cow.id].lat : lastRecord.latitude,
							gps_lon: oof ? outOfFencePos[cow.id].lon : lastRecord.longitude,
							gps_speed: 0,
							gps_accuracy: 5,
							gps_altitude: 300,
							time: lastRecord.datetime,
							alarm: oof ? 'out of fence' : 'ok',
							led_color: oof ? '#ff0000' : (collarInfo?.led_color || '#00ff00'),
	...
	function updateMarkerForCow(cow) {
		...
		const combinedIcon = L.divIcon({
			...
			html: `
			<div class="icon-container${cow.alarm && cow.alarm !== 'ok' ? ' alarm-active' : ''}">
	...
	function hideAllPanels() {
		fencesPanel.classList.add('hidden');
		herdsPanel.classList.add('hidden');
		// Details panel is NOT hidden here — user closes it manually
	}

	// Helper function to display fences
	function displayFences() {
		const colorMap = { green: '#4ade80', red: '#ef4444', blue: '#3b82f6', orange: '#f97316', yellow: '#fbbf24' };
		fenceList.innerHTML = '';

		// Section header: Fences
		const fencesHeader = document.createElement('div');
		fencesHeader.className = 'fence-section-header';
		fencesHeader.textContent = 'Fences';
		fenceList.appendChild(fencesHeader);

		Object.values(fenceData).forEach(fence => {
			const dotColor = colorMap[fence.color] || fence.color;
			const li = document.createElement('li');
			li.className = 'fence-item';
			li.innerHTML = `
				<div class="fence-color" style="background-color: ${dotColor};"></div>
				<div class="fence-info">
					<h4>${fence.name}</h4>
					<p>${fence.cows.length} cows &nbsp;&middot;&nbsp; ID: ${fence.id}</p>
				</div>
			`;
			fenceList.appendChild(li);
		});

		// Section: cows currently outside all fences
		const outOfFenceCows = allCows.filter(c => c.alarm && c.alarm !== 'ok');
		const outsideHeader = document.createElement('div');
		outsideHeader.className = 'fence-section-header fence-section-header--alarm';
		outsideHeader.textContent = `Outside Fences (${outOfFenceCows.length})`;
		fenceList.appendChild(outsideHeader);

		if (outOfFenceCows.length === 0) {
			const empty = document.createElement('p');
			empty.className = 'fence-empty';
			empty.textContent = 'All cows are within fences';
			fenceList.appendChild(empty);
		} else {
			outOfFenceCows.forEach(cow => {
				const li = document.createElement('li');
				li.className = 'fence-item fence-item--alarm';
				li.innerHTML = `
					<div class="fence-color" style="background-color: #ef4444;"></div>
					<div class="fence-info">
						<h4>${cow.name}</h4>
						<p>ID: ${cow.id} &nbsp;&middot;&nbsp; ${cow.alarm}</p>
					</div>
				`;
				fenceList.appendChild(li);
			});
	...
	// Helper function to display herds
	function displayHerds() {
		const colorMap = { brown: '#92400e', black: '#374151', purple: '#7c3aed', green: '#4ade80', red: '#ef4444' };
		herdList.innerHTML = '';
		Object.values(herdData).forEach(herd => {
			const dotColor = colorMap[herd.color] || herd.color;
			const li = document.createElement('li');
			li.className = 'herd-item';
			li.innerHTML = `
				<div class="herd-color" style="background-color: ${dotColor};"></div>
				<div class="herd-info">
					<h4>${herd.name}</h4>
					<p>${herd.cows.length} cows &nbsp;&middot;&nbsp; ID: ${herd.id}</p>
				</div>
			`;
			herdList.appendChild(li);
		});
